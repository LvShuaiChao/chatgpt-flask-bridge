...
const target = path.resolve('tampermonkey-userscript-src/upload/upload-persist-db.js');
...
if (unique.length > 0) {
  console.error('[UPLOAD_PERSIST_DB][FREE_VARS_FOUND]');
  unique.forEach((name) => console.error(`- ${name}`));
  process.exit(1);
}
console.log('[UPLOAD_PERSIST_DB][NO_FREE_VARS_OK]');

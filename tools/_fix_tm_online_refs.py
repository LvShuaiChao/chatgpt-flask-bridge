...
for path in UI.rglob("*.py"):
    ...
    if "bridge_status_online(" in text and "from app.utils.page_snapshot import bridge_status_online" not in text:
        ...
        for i, line in enumerate(lines):
            if line.startswith(("from app.", "import app.")):
                insert_at = i + 1
        if insert_at:
            lines.insert(insert_at, "from app.utils.page_snapshot import bridge_status_online")
            text = "\n".join(lines) + ("\n" if text.endswith("\n") else "")
    if text != orig:

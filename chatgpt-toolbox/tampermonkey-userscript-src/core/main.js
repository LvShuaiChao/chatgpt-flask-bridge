...
// ==/UserScript==
  ...
  function hasPendingReply(sessionId) {
    ...
    try {
      ...
      const errText = e && e.message ? e.message : String(e);
      console.warn('[CHAT_QUEUE][PENDING_CHECK_PERF_INC_FAILED]', e);
      if (typeof ToolboxShell !== 'undefined' && typeof ToolboxShell.appendLog === 'function') {
        ToolboxShell.appendLog(`[CHAT_QUEUE][PENDING_CHECK_PERF_INC_FAILED] error=${errText}`);
      }
    }
  ...
  function buildPageCapabilityCacheKey() {
    return [
      location.href || '',
      document.visibilityState || '',
      document.hasFocus() ? 1 : 0,
  ...
  function getPageCapability(reason = '') {
    const now = Date.now();
    const cacheKey = buildPageCapabilityCacheKey();
    if (

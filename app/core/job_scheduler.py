...
CHATGPT_CURSOR_PROMPT_TEMPLATE = """请根据下面需求，整理成适合直接发给 Cursor Agent / Claude Code 执行的详细修改指令。

必须基于现有代码继续修改，不要重构无关模块，不要删除无关功能。
输出必须尽可能详细，不能只给概要。必须包含：
1. 修改目标：
   - 说明这次要解决什么问题。
   - 说明用户看到的现象。
   - 说明修改后的目标行为。
2. 涉及文件：
   - 写出明确的文件路径。
   - 如果路径不确定，给出搜索关键词和可能所在模块。
3. 具体定位：
   - 写出函数名、类名、变量名、配置字段、状态字段、DOM id、按钮 id、日志标签。
   - 如果涉及状态机，写出 phase/state 的变化。
   - 如果涉及 UI，写出按钮文案、按钮 id、data-action 或相关选择器。
4. 修改前问题：
   - 说明当前逻辑为什么会出错。
   - 如果是异步问题，说明哪个阶段提前执行或重复执行。
   - 如果是状态不同步，说明哪些状态源不一致。
5. 修改后逻辑：
   - 明确新增、替换、删除哪些逻辑。
   - 说明关键判断条件。
...
6. 代码要求：
   ...
   - 代码量较大时，给出 Cursor 可按步骤执行的替换方案。
   - 不要只写“增加判断”“优化逻辑”“完善日志”。
7. 日志要求：
   - 明确要新增或修改的日志标签。
   - 日志应包含 source、reason、runId、round、phase、count、error message 等关键字段。
8. 验证步骤：
   - 给出手动验证步骤。
   - 给出预期日志。
   - 给出异常场景验证。
   - 给出构建或测试命令。
9. 禁止事项：
   - 不要使用 try/except pass。
   - 如果必须捕获异常，必须打印或记录具体错误。
   - 不要引入无关重构。
   - 不要破坏现有功能。

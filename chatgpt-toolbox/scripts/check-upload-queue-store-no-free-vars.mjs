...
const target = path.resolve('tampermonkey-userscript-src/upload/upload-queue-store.js');
...
if (unique.length > 0) {
  console.error('[UPLOAD_QUEUE_STORE][FREE_VARS_FOUND]');
  unique.forEach((name) => console.error(`- ${name}`));
  process.exit(1);
}
console.log('[UPLOAD_QUEUE_STORE][NO_FREE_VARS_OK]');
